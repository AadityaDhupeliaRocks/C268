...
def water_color_effect():
    water_effect = cv2.stylization(decode_array_to_img, cv2.sigma_s = 60, sigma_r = 0.6)
...
def hdr():
    hdr_effect = cv2.detailEnhance(decode_array_to_img, cv2.sigma_s = 12, sigma_r = 0.15)
